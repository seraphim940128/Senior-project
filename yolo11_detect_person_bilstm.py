...
class BiLSTMClassifier(nn.Module):
    def __init__(self, feat_dim=256, use_delta=True, hidden=256, num_classes=9, dropout=0.2):
        super().__init__()
        self.roi_head = DSConV(c=960, mid=feat_dim)   # 若降到 256，這裡 mid=256 並更新下方尺寸
        self.use_delta = use_delta # 是否有做一階差分
        in_dim = feat_dim * (2 if use_delta else 1)

        self.bilstm = nn.LSTM(in_dim, hidden, num_layers=2,
                              batch_first=True, bidirectional=True, dropout=dropout)

        # 時間注意力池化（可選，推薦開）
        self.att = nn.Sequential(nn.Linear(2*hidden, 128),
                                 nn.Tanh(),
                                 nn.Linear(128, 1))
    ...
    def forward(self, x):  # x: [B,T,960,3,3] 或 [B,T,960]
        if x.ndim == 5:
            B, T, C, H, W = x.shape
            x = x.view(B*T, C, H, W)
            x = self.roi_head(x)         # [B*T, C]
            x = x.view(B, T, -1)         # [B,T,C]
        # else: 已是 [B,T,C]，跳過 ROI head

        # 影片內標準化 + Δt
        x = x - x.mean(dim=1, keepdim=True)
        x = x / x.std(dim=1, keepdim=True).clamp_min(1e-6)
        x = temporal_cutout(x, drop_ratio=0.3, train=self.training)  # 隨機 temporal cutout
        if self.use_delta:
            dx = torch.diff(x, dim=1, prepend=x[:, :1, :])
            x = torch.cat([x, dx], dim=2)  # [B,T, 2*C]

        y, _ = self.bilstm(x)             # [B,T,2H]
        w = torch.softmax(self.att(y).squeeze(-1), dim=1)   # [B,T]
        h = (y * w.unsqueeze(-1)).sum(dim=1)                # [B,2H]
        return self.fc(h)
...
def run_one_fold(fold_id, train_idx, val_idx):
    ...
    for epoch in tqdm(range(EPOCHS), desc=f"[Fold {fold_id}] Total Epochs"):
        ...
        matrix_root = "confusion_matrix_bilstm_lse"
        ...
        if val_acc > best_val_acc:
            ...
            torch.save(model.state_dict(), f"best_bilstm_model_lse_fold_{fold_id}.pt")
        else:
            patience_counter += 1
            print(val_acc, best_val_acc, patience_counter)
