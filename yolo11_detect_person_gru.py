...
class VideoDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        meta = {
            ...
            "cached_shape": [960, 7, 7],   # 每幀 shape
            "head": "DSConv+GAP",
            "version": "v10"
        }

        cache_path = make_cache_path(video_path, do_flip, meta, cache_root=os.path.join("data","video_features_v10"))
        ...
        try:
            with np.load(cache_path, allow_pickle=False, mmap_mode="r") as z:
                ...
                ok = (
                    ...
                    (arr.ndim == 4 and arr.shape[1] == 960 and tuple(arr.shape[2:]) == (3, 3)) or
                    (arr.ndim == 4 and arr.shape[1] == 960 and tuple(arr.shape[2:]) == (7, 7))
                )
                if ok:
        ...
        if not cap.isOpened():
            video_feature = torch.zeros((T, 960, 7, 7), dtype=torch.float32)
            detection_ratio = 0.0
...
class GRUClassifier(nn.Module):
    def __init__(self, mid=256, hidden=256, num_layers=2, bidirectional=False,
                 dropout=0.2, num_classes=9, use_delta=True, raw_dim=960):
        super().__init__()
        ...
        # ROI 3x3 -> mid
        self.roi_head = DSConV(c=raw_dim, mid=mid)   # [B*T,960,3,3] -> [B*T, mid]

        # 相容舊特徵 [B,T,960] 用：960 -> mid
        self.adapt = nn.Linear(raw_dim, mid)

        in_dim = mid * (2 if use_delta else 1)
        ...
        hdim = hidden * (2 if bidirectional else 1)
    ...
    def forward(self, x):  # x: [B,T,960,3,3] 或 [B,T,C]
        if x.ndim == 5:
            B, T, C, H, W = x.shape
            x = x.view(B*T, C, H, W).contiguous()   # 保險起見
            x = self.roi_head(x)                    # [B*T, mid]
            x = x.contiguous().view(B, T, self.mid)
        elif x.ndim == 3:
            if x.shape[-1] != self.mid:
                x = self.adapt(x)                   # [B,T,960] -> [B,T,mid]
        else:
        ...
        y, _ = self.gru(x)                          # [B,T, hdim]
        w = torch.softmax(self.att(y).squeeze(-1), dim=1)  # [B,T]
        h = (y * w.unsqueeze(-1)).sum(dim=1)               # [B, hdim]
...
def run_one_fold(fold_id, train_idx, val_idx):
    ...
    model = GRUClassifier(
        mid=256, hidden=256, num_layers=2, bidirectional=False,
        dropout=0.2, num_classes=len(LABELS), use_delta=True
    )
    ...
    for epoch in tqdm(range(EPOCHS), desc=f"[Fold {fold_id}] Total Epochs"):
        ...
        matrix_root = "confusion_matrix_gru_lse"
        ...
        if val_acc > best_val_acc:
            ...
            torch.save(model.state_dict(), f"best_gru_model_lse_fold_{fold_id}.pt")
        else:
            patience_counter += 1
            print(val_acc, best_val_acc, patience_counter)
