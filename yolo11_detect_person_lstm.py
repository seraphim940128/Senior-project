...
class LSTMClassifier(nn.Module):
    def __init__(self, feat_dim=256, raw_dim=960, use_delta=True,
                 hidden=256, num_classes=9, dropout=0.2):
        super().__init__()
        ...
        # ROI 3x3 -> feat_dim
        self.roi_head = DSConV(c=raw_dim, mid=feat_dim)  # [B*C,960,3,3] -> [B*C,feat_dim]

        # 給「直接輸入 [B,T,960]」時用的相容投影（960 -> feat_dim）
        self.adapt = nn.Linear(raw_dim, feat_dim)

        in_dim = feat_dim * (2 if use_delta else 1)
        ...
        self.lstm = nn.LSTM(
            ...
            batch_first=True,
            bidirectional=False,
            dropout=dropout  # 只有 layers>1 才會生效；此處=2會生效
        )
    ...
    def forward(self, x):  # x: [B,T,960,3,3] 或 [B,T,960]
        ...
        # 隨機 temporal cutout（僅訓練）
        x = temporal_cutout(x, drop_ratio=0.15, train=self.training)

        # Δt（可開關）
        if self.use_delta:
            dx = torch.diff(x, dim=1, prepend=x[:, :1, :])
            x = torch.cat([x, dx], dim=2)  # [B,T, 2*feat_dim]

        y, _ = self.lstm(x)                # [B,T,H]（單向）
        w = torch.softmax(self.att(y).squeeze(-1), dim=1)  # [B,T]
        h = (y * w.unsqueeze(-1)).sum(dim=1)               # [B,H]
        return self.fc(h)
...
transform_norm = transforms.Compose([
    transforms.ToTensor(),
    transforms.Normalize(mean=[0.485, 0.456, 0.406], # meta: norm
...
def run_one_fold(fold_id, train_idx, val_idx):
    ...
    for epoch in tqdm(range(EPOCHS), desc=f"[Fold {fold_id}] Total Epochs"):
        ...
        matrix_root = "confusion_matrix_lstm_lse"
        ...
        if val_acc > best_val_acc:
            ...
            torch.save(model.state_dict(), f"best_lstm_model_lse_fold_{fold_id}.pt")
        else:
            patience_counter += 1
            print(val_acc, best_val_acc, patience_counter)
