...
class BiGRUClassifier(nn.Module):
    def __init__(self, mid=256, hidden=256, num_layers=2, bidirectional=True,
                 dropout=0.2, num_classes=9, use_delta=True):
        super().__init__()
        # ROI 3x3 → mid（例如 256）
        self.roi_head = DSConV(c=960, mid=mid)     # 輸入 [B*T,960,3,3] → [B*T, mid]
    ...
    def forward(self, x):  # x: [B,T,960,3,3] 或 [B,T,mid]
        ...
        # 訓練時可做 temporal cutout（放在 Δt 之前）
        if self.training:
...
def run_one_fold(fold_id, train_idx, val_idx):
    ...
    model = BiGRUClassifier(
        mid=256, hidden=256, num_layers=2, bidirectional=True,
    ...
    for epoch in tqdm(range(EPOCHS), desc=f"[Fold {fold_id}] Total Epochs"):
        ...
        matrix_root = "confusion_matrix_bigru_lse"
        ...
        if val_acc > best_val_acc:
            ...
            torch.save(model.state_dict(), f"best_bigru_model_lse_fold_{fold_id}.pt")
        else:
            patience_counter += 1
            print(val_acc, best_val_acc, patience_counter)
