...
def analyze_arm_positions(pose, w, h):
    ...
    if wrist_l is not None and wrist_r is not None:
        wrist_distance = get_distance(wrist_l, wrist_r, w, h)
        if wrist_distance < 0.08 * w: 
            results.append("Palms Together")
        else:
            results.append("Palms Not Together")
    else:
        results.append("Palms Together: Not Visible")
...
def elbow_flexion(shoulder, elbow, wrist, w, h, img, elbow_init=(0,0,0), step=-1, round=0, start=False, start_time=0):
    max_upper_arm_shift, max_upper_elbow_shift = 0.15, 0.05
    ...
    if step == 0 and 40 < angle_elbow < 80 and start:
...
def shoulder_abduction(landmarks, side, w, h, step, round, start, start_time):
    ...
    elif step == 0 and 85 < angle_shoulder < 95 and start and depth_diff < max_depth_diff: ######
        if current_time - start_time > 2:
            step, start_time = 1, current_time
...
def shoulder_forward_elevation_logic(landmarks, w, h, step, round, start, start_time):
    now = time.time()
    analysis = analyze_arm_positions(landmarks, w, h)
    head_dir, _ = get_head_direction(landmarks)
    pose_up  = detect_pose(analysis, head_dir, target_pose_name="Shoulder Forward Elevation")
    error_msg = ""
...
def side_tap(landmarks, side, w, h, step, round, start, start_time):
    ...
    if not is_arm_straight(left_hip, left_knee, left_ankle) or not is_arm_straight(right_hip, right_knee, right_ankle):
...
exercises = {
    ...
    "elbow_flexion_left": {"elbow_init": (0, 0, 0), "step": -1, "round": 0, "start": False, "start_time": 0, "color": (128, 0, 128)},
    "elbow_flexion_right": {"elbow_init": (0, 0, 0), "step": -1, "round": 0, "start": False, "start_time": 0, "color": (128, 0, 128)},
}
